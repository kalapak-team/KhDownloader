...
import { convertExcelToPdf } from "../services/api";
    ...
    >
      ...
      <div>
        <p className="text-base font-semibold text-textPrimary">
          Drop your Excel file here or <span className="text-[#e8472a]">browse</span>
        </p>
        <p className="mt-1 text-sm text-textSecondary">XLS & XLSX files only · Max 50 MB</p>
      </div>
      <input
        ref={inputRef}
        type="file"
...
export default function ExcelToPdf() {
  ...
  const handleFile = useCallback((f) => {
    ...
    if (!name.endsWith(".xls") && !name.endsWith(".xlsx")) {
      toast.error("Only XLS and XLSX files are accepted");
      return;
    }
    setFile(f);
    setStatus("idle");
  ...
  const handleConvert = async () => {
    ...
    setStatus("uploading");
    try {
      const { filename, data } = await convertExcelToPdf(file);
