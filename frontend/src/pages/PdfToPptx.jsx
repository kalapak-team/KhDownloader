...
export default function PdfToPptx() {
  ...
  const handleConvert = async () => {
    ...
    setStatus("uploading");
    try {
      const { filename, data } = await convertPdfToPptx(file);
  ...
  const handleDownload = async () => {
    try {
      ...
      if (window.showSaveFilePicker) {
        try {
          const fileHandle = await window.showSaveFilePicker({
            suggestedName: downloadName || "converted.pptx",
      ...
      a.href = url;
      a.download = downloadName || "converted.pptx";
      ...
      <div className="animate-fade-up-d1 mx-auto w-full max-w-xl rounded-2xl border border-[rgba(255,255,255,0.07)] bg-[rgba(255,255,255,0.03)] p-6 backdrop-blur-md">
        ...
        {status === "done" && (
          <div className="flex flex-col items-center gap-4 py-6">
            ...
            <p className="text-sm text-textSecondary">Your PowerPoint file is ready to download.</p>
            <button
              type="button"
      ...
      <div className="animate-fade-up-d2 mx-auto w-full max-w-xl">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          {[
            ...
            { step: "2", title: "Convert", desc: "Each page becomes a high-quality PowerPoint slide." },
            { step: "3", title: "Download", desc: "Download your PPTX file, ready to present." },
          ].map(({ step, title, desc }) => (
            <div
              key={step}
