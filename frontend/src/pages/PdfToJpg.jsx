...
import { FileImage, Upload, X, CheckCircle2, AlertCircle, Loader2, FileText } from "lucide-react";
import toast from "react-hot-toast";
import { convertPdfToJpg } from "../services/api";
    ...
    >
      ...
      <div>
        <p className="text-base font-semibold text-textPrimary">
          Drop your PDF here or <span className="text-[#e8472a]">browse</span>
        </p>
...
export default function PdfToJpg() {
  ...
  const handleConvert = async () => {
    ...
    setProgress(0);
    try {
      const { filename, data } = await convertPdfToJpg(file);
      // Decode base64 → Blob in memory. IDM never sees this — it's pure JS.
  ...
  const handleDownload = async () => {
    try {
      ...
      if (!blob || blob.size === 0) { toast.error("No file ready — please convert first"); return; }

      // Convert to ArrayBuffer — passing a Blob directly to writable.write() is
      // bugged in some Chromium versions and can write 0 bytes.
      const buffer = await blob.arrayBuffer();

      // showSaveFilePicker writes directly to disk — IDM cannot intercept this
      if (window.showSaveFilePicker) {
        try {
          const fileHandle = await window.showSaveFilePicker({
            suggestedName: downloadName || "pages.zip",
          ...
          if (e.name === "AbortError") return; // user cancelled the Save As dialog
      ...
      a.href = url;
      a.download = downloadName || "pages.zip";
    ...
    <div className="grid gap-6 py-10 pb-16">
      ...
      <div className="animate-fade-up text-center">
        ...
        <p className="animate-fade-up-d1 mt-2 text-sm text-textSecondary">
          Convert each PDF page into a JPG image. All pages are packaged into a ZIP file.
        </p>
      </div>
      ...
      <div className="animate-fade-up-d1 mx-auto w-full max-w-xl rounded-2xl border border-[rgba(255,255,255,0.07)] bg-[rgba(255,255,255,0.03)] p-6 backdrop-blur-md">
        ...
        {/* Drop zone (only when no file or idle) */}
        {(!file || status === "idle") && !file && (
          <DropZone onFile={handleFile} />
        )}

        {/* Drop zone replacement when file loaded */}
        {file && status === "idle" && (
          <DropZone onFile={handleFile} />
        )}
        ...
        {status === "uploading" && (
          <div className="flex flex-col items-center gap-4 py-8">
            ...
            <p className="text-sm font-medium text-textPrimary">Converting… {progress}%</p>
            <div className="h-1.5 w-full overflow-hidden rounded-full bg-[rgba(255,255,255,0.08)]">
              <div
                className="h-full rounded-full bg-[#e8472a] transition-all duration-300"
        ...
        {status === "done" && (
          <div className="flex flex-col items-center gap-4 py-6">
            ...
            <p className="text-sm text-textSecondary">Your JPG pages are ready in a ZIP file.</p>
            <button
              type="button"
      ...
      <div className="animate-fade-up-d2 mx-auto w-full max-w-xl">
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          {[
            ...
            { step: "2", title: "Convert", desc: "Each page is rendered as a high-quality JPG image." },
            { step: "3", title: "Download ZIP", desc: "All JPG pages are bundled into one ZIP for you." },
          ].map(({ step, title, desc }) => (
            <div
              key={step}
