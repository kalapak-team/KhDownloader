...
import { AlertCircle, CheckCircle2, FileText, Loader2, Presentation, Upload, X } from "lucide-react";
import toast from "react-hot-toast";
import { convertPptxToPdf } from "../services/api";
    ...
    >
      ...
      <div>
        <p className="text-base font-semibold text-textPrimary">
          Drop your PPT/PPTX file here or <span className="text-[#e8472a]">browse</span>
        </p>
        <p className="mt-1 text-sm text-textSecondary">PPT & PPTX files only · Max 50 MB</p>
      </div>
      <input
        ref={inputRef}
        type="file"
...
export default function PptxToPdf() {
  ...
  const handleFile = useCallback((f) => {
    ...
    if (!name.endsWith(".ppt") && !name.endsWith(".pptx")) {
      toast.error("Only PPT and PPTX files are accepted");
      return;
    }
    setFile(f);
    setStatus("idle");
  ...
  const handleConvert = async () => {
    ...
    setStatus("uploading");
    try {
      const { filename, data } = await convertPptxToPdf(file);
  ...
  return (
    <div className="grid gap-6 py-10 pb-16">
      ...
      <div className="animate-fade-up-d1 mx-auto w-full max-w-xl rounded-2xl border border-[rgba(255,255,255,0.07)] bg-[rgba(255,255,255,0.03)] p-6 backdrop-blur-md">
        ...
        {status === "done" && (
          <div className="flex flex-col items-center gap-4 py-6">
            ...
            <p className="text-sm text-textSecondary">Your PDF file is ready to download.</p>
            <button
              type="button"
      ...
      <div className="animate-fade-up-d2 mx-auto grid w-full max-w-xl grid-cols-3 gap-4">
        {[
          { step: "1", label: "Upload PPTX" },
          { step: "2", label: "Convert" },
          { step: "3", label: "Download PDF" },
        ].map((item) => (
